...
let uri = "mongodb+srv://Mike:9XroKvnSqRE3WbWe@cluster0-lyff2.mongodb.net/freecodecamp?retryWrites=true&w=majority";
...
app.get('/api/exercise/log',(request,response)=>{
    User.findById(request.query.userId,(error,result)=>{
      if(!error){
        ...
        if(request.query.form || request.query.to){
          let formDate = new Date(0)
          let toDate = new Date()
          
          if(request.query.form){
            formDate = new Date(request.query.form)
          }
          if(request.query.to){
            toDate = new Date(request.query.to)
          }
           formDate = formDate.getTime()
           toDate = toDate.getTime()
          responseObject.log = responseObject.log.filter((session)=>{
                let sessionDate = new Date(session.date).getTime()
                
                return sessionDate >= formDate && sessionDate <= toDate
          })
        }
